...
const CreateCommunityCategory = () => {
  ...
  const fetchCategories = async () => {
    try {
      const response = await axios.get(
        "http://localhost:3000/api/FetchCommunityCategories"
      );
  ...
  const validateAndSetJsonData = (jsonData) => {
    try {
      ...
      if (!jsonData.categoryName || !Array.isArray(jsonData.questions)) {
        throw new Error(
          "Estructura JSON inválida. Debe contener 'categoryName' y 'questions'"
        );
      }

      if (jsonData.questions.length !== QUESTION_COUNT) {
        throw new Error(
          `El archivo debe contener exactamente ${QUESTION_COUNT} preguntas`
        );
      }
      ...
      if (
        jsonData.gameMode &&
        ["Bullet", "Solitary", "Online"].includes(jsonData.gameMode)
      ) {
  ...
  const downloadExampleJson = () => {
    const exampleData = {
      ...
      gameMode: "Solitary",
      questions: Array(QUESTION_COUNT)
        .fill()
        .map((_, index) => ({
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (getCompletedQuestionCount() !== QUESTION_COUNT) {
        setMessage(
          `Por favor, completa todas las preguntas (${getCompletedQuestionCount()}/${QUESTION_COUNT} completadas)`
        );
        setStatus("error");
        return;
      }
    ...
    return (
      <div className={styles.categoryContainer}>
        <BackButton
          ...
          ariaLabel="Volver a comunidades"
        />
          ...
          <div className={styles.modeSelection}>
            ...
            <div className={styles.helpSection}>
              ...
              <p className={styles.helpText}>
                ¿No tienes un archivo JSON? Descarga este ejemplo para ver el
                formato correcto.
              </p>
            </div>
          </div>
    ...
    return (
      <div className={styles.categoryContainer}>
        <BackButton
          ...
          ariaLabel="Volver a selección de modo"
        />
          ...
          <div className={styles.fileUploadSection}>
            <div className={styles.uploadArea}>
              ...
              <label htmlFor="jsonFile" className={styles.fileLabel}>
                ...
                <p>
                  El archivo debe contener una categoría con exactamente{" "}
  ...
  "questions": [
    ...
    // ... 9 preguntas más
              ...
              <p>
                <strong>Nota:</strong> El campo "gameMode" es opcional. Valores
                válidos: "Solitary", "Online", "Bullet"
              </p>
            </div>
