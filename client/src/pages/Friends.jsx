...
function Friends() {
  ...
  const fetchPendingRequests = async () => {
    try {
      ...
      const response = await axios.get(
        "http://localhost:3000/api/pendingRequests",
        {
          params: { userId },
        }
      );
  ...
  const acceptFriendRequest = async (requestId) => {
    try {
      ...
      setPendingRequests(
        pendingRequests.filter((req) => req.request_id !== requestId)
      );
  ...
  const sendFriendRequest = async () => {
    ...
    try {
      ...
      const friendIdResponse = await axios.get(
        "http://localhost:3000/api/getFriendId/",
        {
      ...
      const response = await axios.post(
        `http://localhost:3000/api/friendRequest:id`,
        {
  ...
  const deleteFriend = async (friend) => {
    try {
      ...
      const response = await axios.delete(
        "http://localhost:3000/api/deleteFriend:id",
        {
  ...
  return (
    <div className={styles.pageContainer}>
      <div className={styles.friendsContainer}>
        <BackButton onClick={handleGoBack} ariaLabel="Volver atrás" />
                    ...
                    <li key={request.request_id} className={styles.requestItem}>
                      <div>
                        <img
                          ...
                          src={
                            request.profileImage || "/defaultProfileImage.png"
                          }
                          alt={`${request.sender_username} avatar`}
                        />
                      </div>
                  ...
                  <li key={friend.user_id} className={styles.friendItem}>
                    <div className={styles.friendAvatar}>
                      <img
                        ...
                        src={friend.profileImage || "/defaultProfileImage.png"}
