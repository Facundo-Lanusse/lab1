...
function Profile() {
  ...
  const handleImageUpload = async () => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:3000/api/editProfile/upload-image",
        formData,
        {
          headers: {
      ...
      if (response.data.success) {
        ...
        setMessage({
          text: response.data.message || "Error al subir la imagen",
          isError: true,
        });
      }
    } catch (error) {
  ...
  const handleUsernameSubmit = async () => {
    ...
    try {
      const res = await axios.put(
        "http://localhost:3000/api/editProfile/username",
        {
  ...
  const handleEmailSubmit = async () => {
    ...
    try {
      const res = await axios.put(
        "http://localhost:3000/api/editProfile/email",
        {
  ...
  const handlePasswordSubmit = async () => {
    ...
    try {
      const res = await axios.put(
        "http://localhost:3000/api/editProfile/password",
        {
  ...
  return (
    ...
    >
      <BackButton onClick={goBack} ariaLabel="Volver atrás" />
