...
function UserRanking() {
    ...
    async (page = 1, search = "", sort = "desc") => {
      try {
        ...
        if (error.response?.status === 401 || error.response?.status === 403) {
          ...
          setError(
            "No se pudieron cargar los datos del ranking. Por favor, intenta de nuevo más tarde."
          );
        }
        setLoading(false);
      }
    ...
    return (
      <div className={styles.container}>
        <div className={styles.errorState}>
          <p>{error}</p>
          <button
            onClick={() => fetchUsers(currentPage, searchTerm, sortDirection)}
  ...
  return (
    <div className={styles.container}>
      ...
      <button className={styles.playButton} onClick={() => navigate("/Play")}>
        <svg
